...
resizeImage = async (event, context) => {
    ...
    const tempFilePath = `/tmp/${uuid.v4()}-${event.name}`;
    ...
    await storage.bucket(event.bucket).upload(resizedImagePath, {
        destination: `resized/${resizedImageName}`,
    });

    // Cleanup: Delete the temporary files
    await Promise.all([file.delete(), sharp.cache(false)]);
    console.log(`Image ${event.name} resized and uploaded with random name ${resizedImageName}.`);
